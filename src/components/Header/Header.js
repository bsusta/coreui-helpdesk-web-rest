...
import { Nav, NavItem, NavbarToggler, NavbarBrand, InputGroup, InputGroupAddon, Input } from 'reactstrap';
	...
	render() {
		return (
			<header className="app-header navbar">
				<SimpleLoadingBar activeRequests={this.props.activeRequests} color="#2A7EC5" />
				<div style={{ width: 260 }}>
					<NavbarToggler className="d-md-down-none" onClick={this.sidebarToggle}>
						<span className="navbar-toggler-icon" />
					</NavbarToggler>
					<NavbarToggler className="headerText">
						<a className="headerIcons" href="#">
							LanHelpdesk
						</a>
					</NavbarToggler>
				</div>
				...
				<div className="ml-auto row headerCommandBar">
					{/*Settings icon*/}
					{this.props.user.user_role.acl.some(item =>
						[
							'company_settings',
						...
						<a className="headerIcons" href="#/settings">
							<i className="fa fa-cog headerIcons" />
						</a>
					)}
					<MessagesDropdown />
					{this.props.errorMessages.length > 0 && <ErrorMessagesDropdown />}
					<a className="d-md-down-none headerIcons" href={'#/user/profile'}>
						{this.props.user.username}
					</a>
					<i className="headerIcons">
						<i className="fa fa-sign-out" onClick={this.props.logoutUser} />
					</i>
				</div>
			</header>
		);
