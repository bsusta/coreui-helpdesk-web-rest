			...
			<div>
				<Nav tabs>
					...
					{(this.props.task.loggedUserIsAdmin ||
						this.props.task.loggedUserProjectAcl.includes('resolve_task')) &&
						this.props.task.loggedUserRoleAcl.includes('sent_emails_from_comments') && (
							<NavItem>
								<NavLink
				...
				>
					<TabPane tabId="1" style={{ paddingLeft: 0, paddingRight: 0 }}>
						...
						<div className="form-group">
							<input
								type="file"
								id={this.props.commentID ? 'addAttachment' + this.props.commentID : 'addAttachment'}
								style={{ display: 'none' }}
								onChange={e => {
							...
							<label
								...
								size="sm"
								htmlFor={
									this.props.commentID ? 'addAttachment' + this.props.commentID : 'addAttachment'
								}
								style={{ cursor: 'pointer', color: colors.textBlue }}
							>
								<i style={{ color: colors.textBlue }} className="fa fa-paperclip" />
								<span style={{ color: colors.textBlue }}> {i18n.t('addAttachment')} </span>
							</label>
							{(this.props.task.loggedUserIsAdmin ||
								this.props.task.loggedUserProjectAcl.includes('edit_internal_note')) && (
								<Label
									check
									htmlFor={this.props.commentID ? 'internal' + this.props.commentID : 'internal'}
									style={{ marginLeft: 5 }}
						...
						>
							...
							<div style={{ paddingTop: 5, paddingRight: 10 }}>
								{this.props.commentAttachments.map(item => (
									...
									>
										<div style={{ marginTop: 'auto', marginBottom: 'auto' }}>{item.file.name}</div>
										<div style={{ flex: 1 }} />
					...
					>
						...
						<FormGroup row>
							...
							<Col xs="12" md="10">
								...
								<Input
									...
									onChange={e => this.setState({ newCC: e.target.value })}
								/>
							</Col>
						</FormGroup>
						...
						<FormGroup row>
							...
							<Col xs="12" md="10">
								...
								<Input
									...
									onChange={e => this.setState({ newBCC: e.target.value })}
								/>
							</Col>
						</FormGroup>
						...
						<div className="form-group">
							...
							>
								<i style={{ color: colors.textBlue }} className="fa fa-paperclip" />
								<span style={{ color: colors.textBlue, paddingRight: 5 }}>
									{' '}
...
const mapStateToProps = ({ login, usersReducer, commentAttachmentsReducer, tasksReducer }) => {
