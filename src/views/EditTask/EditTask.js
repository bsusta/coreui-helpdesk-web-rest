...
class EditTask extends Component {
	constructor(props) {
		...
		this.state = {
			company,
			deadline: this.props.task.deadline ? moment(this.props.task.deadline * 1000) : null,
			closedAt: this.props.task.closedAt ? moment(this.props.task.closedAt * 1000) : null,
			startedAt: this.props.task.startedAt ? moment(this.props.task.startedAt * 1000) : null,
			...
			tags: this.props.task.tags
				.filter(tag => this.props.tags.findIndex(item => item.id === tag.id) != -1)
				.map(tag => tag.id),
	...
	autoSubmit(name, value, id) {
		...
		if (name === 'project') {
			...
			state['company'] = this.props.companies[
				this.props.companies.findIndex(item => item.id === value.company.id)
			];
		} else if (name === 'status') {
		...
		this.props.editTask(
			{
				...
				closedAt: state.closedAt !== null ? state.closedAt.valueOf() / 1000 : 'null',
				...
				<Card>
					...
					>
						<div>
							...
							{(this.props.task.loggedUserProjectAcl.includes('delete_task') ||
								this.props.task.loggedUserIsAdmin) && (
						...
						{this.state.submitError && (
							<span>
								<h5 style={{ color: 'red' }}> {i18n.t('restrictionTaskWontSave')}</h5>
							</span>
						)}
					</CardHeader>
					<CardBody>
						<div className="row task-header">
							...
							>
								<label className="float- input-label">
									Vytvoril: {this.props.task.createdBy.username} ({this.props.task.createdBy.email}){' '}
						...
						<div>
							<div className="row">
								<div className="col-8 task-edit-col-1">
									...
									<SubtasksLoader taskID={this.props.task.id} units={this.props.units} />
								...
								<div className="col-4 task-edit-col-2">
									<FormGroup>
										...
										{this.state.closedAt && (
											<span style={{ float: 'right' }}>
												{i18n.t('changedAt')}: {timestampToString(this.state.closedAt / 1000)}
											</span>
										)}
										<InputGroup>
											<InputGroupAddon>
											...
											{this.state.status && (
												...
												>
													<i className="fa fa-circle" style={{ paddingTop: 10 }} />
												</span>
											)}
											<select
												className="form-control"
									...
									<FormGroup>
										...
										<InputGroup
											className={this.state.requestedBy.id === undefined ? 'fieldError' : ''}
										>
											<InputGroupAddon>
