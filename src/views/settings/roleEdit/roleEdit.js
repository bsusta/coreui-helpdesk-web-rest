...
const ACLs=[
  ...
  {value:'create_tasks_in_all_projects',title:'Create tasks in all projects'},
  ...
  render() {
    return (
      ...
      <h3>ACLs</h3>
      {
        ...
        <div class="form-check">
          <label class="form-check-label">
            <input type="checkbox" class="form-check-input" checked={this.state.acl.includes(acl.value)} onChange={()=>this.aclChange(acl.value)}/>
            {acl.title}
          </label>
        </div>
      )
    ...
    <button type="submit" onClick={this.submit.bind(this)} class="btn btn-primary">
    Submit
    </button>
    </form>
    </div>
