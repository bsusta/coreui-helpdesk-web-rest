...
class taskAttributesList extends Component {
  ...
  getFilteredData(){
    return this.props.taskAttributes.filter((item)=>item.title.toLowerCase().includes(this.state.title.toLowerCase()))
          ...
          <tbody>
            ...
            {this.getFilteredData().map(taskAttribute => (
              <tr
                ...
                onClick={() => this.props.history.push("/taskAttribute/edit/" + taskAttribute.id)}
              >
